...
#[test]
fn set_brightness_temp_percent_with_kelvin() {
    let out = rmod(&["set", "-b", "50", "-t", "3400", "-y"]);
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let text = stdout(&out);
    assert!(text.contains("50%"), "stdout: {text}");
    assert!(text.contains("3400K"), "stdout: {text}");
    // Backend can be ddc, slider, or gamma depending on the fake monitor
    assert!(text.contains("via"), "stdout: {text}");
}

#[test]
fn set_brightness_temp_min_with_preset() {
    let out = rmod(&["set", "-b", "min", "-t", "warm", "-y"]);
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let text = stdout(&out);
    assert!(text.contains("min"), "stdout: {text}");
    assert!(text.contains("2700K"), "stdout: {text}");
    assert!(text.contains("gamma"), "stdout: {text}"); // min mode always uses gamma layer
}

#[test]
fn set_brightness_temp_max_with_alias() {
    let out = rmod(&["set", "-b", "max", "-t", "incandescent", "-y"]);
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let text = stdout(&out);
    assert!(text.contains("max"), "stdout: {text}");
    assert!(text.contains("2700K"), "stdout: {text}");
    assert!(text.contains("gamma 100%"), "stdout: {text}"); // max mode always uses gamma 100%
}

#[test]
fn set_brightness_temp_boost_with_kelvin() {
    let out = rmod(&["set", "-b", "boost", "-t", "4000", "-y"]);
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let text = stdout(&out);
    assert!(text.contains("boost"), "stdout: {text}");
    assert!(text.contains("4000K"), "stdout: {text}");
    assert!(text.contains("gamma 130%"), "stdout: {text}"); // boost mode always uses gamma 130%
}

#[test]
fn set_brightness_temp_with_monitor() {
    let out = rmod(&["set", "-b", "50", "-t", "3400", "-m", "2", "-y"]);
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let text = stdout(&out);
    assert!(text.contains("RMOD Fake Monitor 2 [:2]"), "stdout: {text}");
    assert!(text.contains("50%"), "stdout: {text}");
...
fn set_brightness_temp_with_all() {
    let out = rmod(&["set", "-b", "50", "-t", "3400", "-m", "all", "-y"]);
    assert_eq!(out.status.code(), Some(0), "stderr: {}", stderr(&out));
    let text = stdout(&out);
    assert!(text.contains("RMOD Fake Monitor 1 [:1]"), "stdout: {text}");
    assert!(text.contains("RMOD Fake Monitor 2 [:2]"), "stdout: {text}");
    assert!(text.contains("50%"), "stdout: {text}");
    assert!(text.contains("3400K"), "stdout: {text}");
}
...
fn set_brightness_alone_errors() {
    let out = rmod(&["set", "-b", "50"]);
    assert_eq!(out.status.code(), Some(2));
    assert!(stderr(&out).contains("use rmod monitor brightness for brightness only"));
}

#[test]
fn set_temp_alone_errors() {
    let out = rmod(&["set", "-t", "3400"]);
    assert_eq!(out.status.code(), Some(2));
    assert!(stderr(&out).contains("use rmod temp for temperature only"));
}

#[test]
fn set_brightness_temp_mutual_exclusion_with_resolution() {
    let out = rmod(&["set", "-b", "50", "-t", "3400", "-w", "1920", "-h", "1080"]);
    assert_eq!(out.status.code(), Some(2));
    assert!(
        stderr(&out).contains("use `rmod monitor brightness` / `rmod temp` for single actions; `-b` and `-t` compose")
    );
}

#[test]
fn set_brightness_temp_kelvin_bounds_enforced() {
    for arg in ["999", "6501"] {
        let out = rmod(&["set", "-b", "50", "-t", arg]);
        assert_eq!(out.status.code(), Some(2), "arg: {}", arg);
        assert!(stderr(&out).contains("invalid temperature"), "arg: {}", arg);
    }
}

#[test]
fn set_brightness_temp_brightness_bounds_enforced() {
    let out = rmod(&["set", "-b", "150", "-t", "3400"]);
    assert_eq!(out.status.code(), Some(2));
    assert!(stderr(&out).contains("invalid brightness"));
}
