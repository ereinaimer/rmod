...
pub(crate) fn parse_set(args: &[impl AsRef<str>]) -> Result<Command, String> {
    ...
    while i < args.len() {
        ...
        match arg {
            ...
            other => {
                return Err(format!(
                    "unexpected argument {} for set. use --width, --height, --refresh, --profile, --monitor, --orientation, --max, or --help",
                    other
                ));
            }
        }
...
mod tests {
    ...
    fn set_missing_spec_is_error() {
        assert_eq!(
            parse(&["set"]),
            Err(
                "set needs something to change. width/height, refresh rate, profile, or --max\ne.g. rmod set -p 1080"
                    .to_string()
            )
        );
    }
    ...
    #[test]
    fn set_brightness_flag_is_unknown_argument() {
    ...
    #[test]
    fn set_temp_flag_is_unknown_argument() {
    ...
    #[test]
    fn set_brightness_temp_together_is_unknown_argument() {
        let err = parse(&["set", "-b", "50", "-t", "3400"]).unwrap_err();
        assert!(err.contains("unexpected argument"));
    }

    #[test]
    fn set_long_brightness_flag_is_unknown_argument() {
        let err = parse(&["set", "--brightness", "50"]).unwrap_err();
        assert!(err.contains("unexpected argument"));
        assert!(err.contains("--brightness"));
    }

    #[test]
    fn set_long_temp_flag_is_unknown_argument() {
        let err = parse(&["set", "--temp", "3400"]).unwrap_err();
        assert!(err.contains("unexpected argument"));
