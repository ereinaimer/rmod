//! HDR capability query: the Windows display-config API first, the EDID
//! CTA-861 static metadata as fallback.
//!
//! [`query_hdr`] asks the OS whether the display supports and currently
//! enables HDR via `DisplayConfigGetDeviceInfo`; when that fails (old
//! Windows, headless sessions), [`from_edid`] reports what the panel's EDID
//! advertises. Nothing here panics and every FFI failure degrades to the
//! EDID fallback.
...
/// Queries the HDR capability of the display with Win32 device name
/// `device_name` (e.g. `\\.\DISPLAY1`), falling back to the EDID metadata.
///
/// The display-config API is tried first; any failure at any step returns
/// the EDID fallback instead of an error.
#[allow(dead_code)]
pub(crate) fn query_hdr(device_name: &str, edid_hdr: Option<&edid::HdrEdid>) -> Option<HdrInfo> {
    api_hdr(device_name, edid_hdr).or_else(|| from_edid(edid_hdr))
}
...
/// The Windows display-config API path: enumerates active paths, matches
/// the one whose GDI source device name equals `device_name` (reported by
/// the OS itself via `DisplayConfigGetDeviceInfo`, so no adapter-LUID
/// assumptions), and reads the advanced-color capability addressed by the
/// matched path's target device. `None` on every FFI failure.
fn api_hdr(device_name: &str, edid_hdr: Option<&edid::HdrEdid>) -> Option<HdrInfo> {
